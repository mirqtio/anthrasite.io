...
(async () => {
  ...
  const diagnostics = await page.evaluate(() => {
    return {
      ...
      scripts: Array.from(document.scripts).map(s => ({ src: s.src, inline: !s.src })),
  ...
  const mainResponse = await page.evaluateHandle(() => window.performance.getEntriesByType('navigation')[0]);
  const responseData = await mainResponse.jsonValue();
  console.log('Main page response:', responseData.name, responseData.responseStatus);
