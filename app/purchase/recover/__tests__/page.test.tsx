...
describe('RecoverPage', () => {
  ...
  it('should show already recovered message for recovered carts', async () => {
    ...
    expect(screen.getByText(/This cart has already been recovered/)).toBeInTheDocument()
  })
  ...
  it('should show session expired message for expired sessions', async () => {
    ...
    expect(screen.getByText(/Your checkout session has expired/)).toBeInTheDocument()
    expect(screen.getByRole('link', { name: 'Start New Purchase' })).toHaveAttribute(
      'href',
      '/purchase?utm=utm-123'
    )
  })
  ...
  it('should show session not found message when Stripe session is missing', async () => {
    ...
    expect(screen.getByText(/We couldn't find your checkout session/)).toBeInTheDocument()
  })
  ...
  it('should redirect to Stripe checkout for valid recovery', async () => {
    ...
    expect(redirect).toHaveBeenCalledWith('https://checkout.stripe.com/pay/cs_test_123')
  })
})