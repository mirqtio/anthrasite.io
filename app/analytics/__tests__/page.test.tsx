...
describe('AnalyticsPage', () => {
  ...
  it('should render the analytics dashboard', () => {
    ...
    expect(screen.getByText('Track performance, conversions, and A/B test results')).toBeInTheDocument()
  })
  ...
  it('should render funnel visualization with correct data', () => {
    ...
    // Check funnel steps
    expect(screen.getByTestId('funnel-step-Homepage Visit')).toHaveTextContent('Homepage Visit: 10000 (100%)')
    expect(screen.getByTestId('funnel-step-UTM Validated')).toHaveTextContent('UTM Validated: 7500 (75%)')
    expect(screen.getByTestId('funnel-step-Purchase Page')).toHaveTextContent('Purchase Page: 6000 (60%)')
    expect(screen.getByTestId('funnel-step-Checkout Started')).toHaveTextContent('Checkout Started: 3000 (30%)')
    expect(screen.getByTestId('funnel-step-Payment Completed')).toHaveTextContent('Payment Completed: 1500 (15%)')
  })
  ...
  it('should render A/B test results', () => {
    ...
    // Check variants
    expect(screen.getByTestId('variant-control')).toHaveTextContent('Get Started (Control): 3%')
    expect(screen.getByTestId('variant-variant-a')).toHaveTextContent('Start Free Trial: 3.6%')
    expect(screen.getByTestId('variant-variant-b')).toHaveTextContent('Get Your Audit: 3.2%')
  })
  ...
  it('should have proper layout structure', () => {
    ...
    expect(container.querySelector('main')).toHaveClass('min-h-screen', 'bg-gray-50', 'py-12')
  ...
  it('should display metrics in a grid layout', () => {
    ...
    expect(metricsGrid).toHaveClass('grid-cols-1', 'md:grid-cols-4', 'gap-6', 'mb-8')
  ...
  it('should apply correct styling to headers', () => {
    ...
    expect(mainHeader).toHaveClass('text-3xl', 'font-bold', 'text-anthracite-black')
    
    const subHeader = screen.getByText('A/B Test Results')
    expect(subHeader).toHaveClass('text-2xl', 'font-semibold', 'text-anthracite-black', 'mb-6')
  })
