...
test.describe('Waitlist Form Functionality', () => {
  ...
  test('should successfully submit waitlist form', async ({ page }) => {
    ...
    const waitlistForm = page
      .locator('form')
      .filter({ hasText: 'Enter your website domain' })
    ...
    // Wait for email step
    await expect(
      page.locator('text="Great! We\'ll analyze mycompany.com"')
    ).toBeVisible()
  ...
  test('should validate domain format', async ({ page }) => {
    ...
    // Check for error
    await expect(
      page.locator('text="Invalid characters in domain"')
    ).toBeVisible()

    // Continue button should be disabled
    await expect(
      waitlistForm.locator('button:has-text("Continue")')
    ).toBeDisabled()
  })
  ...
  test('should handle duplicate signups', async ({ page }) => {
    ...
    await expect(
      page.locator(`text="You're number #${firstPosition}"`)
    ).toBeVisible()
  })
})
