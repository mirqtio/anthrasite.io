...
describe('PurchaseHomepage', () => {
  ...
  it('should render all components with business data', () => {
    ...
    expect(screen.getByTestId('purchase-hero')).toHaveTextContent('Test Business')
    expect(screen.getByTestId('report-preview')).toHaveTextContent('Report for testbusiness.com')
  ...
  it('should show loading state during checkout', async () => {
    ...
    ;(createCheckoutSession as jest.Mock).mockImplementation(
      () => new Promise(resolve => setTimeout(() => resolve({ sessionId: 'cs_test_123' }), 100))
    )
  ...
  it('should handle redirect error', async () => {
    mockStripe.redirectToCheckout.mockRejectedValue(new Error('Redirect failed'))
