...
export class FAQService {
  ...
  private getPageSpecificFAQs(page: PageContext): FAQItem[] {
    switch (page) {
      ...
      case PageContext.SUCCESS:
        return [
          ...REPORT_FAQS,
          ...PURCHASE_FAQS.filter(f => f.tags?.includes('sharing') || f.tags?.includes('updates')),
        ];
   ...
   */
  async searchFAQs(query: string, context?: HelpContext): Promise<FAQSearchResult[]> {
    if (!query.trim()) {
      return [];
    }
