...
describe('FAQService', () => {
  ...
  describe('searchFAQs', () => {
    ...
    it('should find FAQs by answer text', async () => {
      ...
      expect(results.length).toBeGreaterThan(0)
      expect(
        results.some((r) => r.item.answer.toLowerCase().includes('performance'))
      ).toBe(true)
    })
    ...
    it('should score question matches higher than answer matches', async () => {
      ...
      const questionMatch = results.find((r) =>
        r.item.question.toLowerCase().includes('report')
      )
      const answerOnlyMatch = results.find(
        (r) =>
          !r.item.question.toLowerCase().includes('report') &&
  ...
  describe('getRelatedFAQs', () => {
    ...
    it('should return FAQs from same category', () => {
      ...
      expect(
        relatedFAQs.some((f) => f.category === FAQCategory.TECHNICAL)
      ).toBe(true)
    })
  ...
  describe('getPopularFAQs', () => {
    ...
    it('should return valid FAQs only', () => {
      ...
      expect(popularFAQs.every((f) => f.id && f.question && f.answer)).toBe(
        true
      )
    })
  })
