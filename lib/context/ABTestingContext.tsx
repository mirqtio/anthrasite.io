...
import { Experiment, VariantAssignment, ABTestContextValue } from '../ab-testing/types';
import { getVariantAssignment, batchAssignVariants } from '../ab-testing/variant-assignment';
import { fetchExperiments, subscribeToExperimentChanges } from '../ab-testing/edge-config';
...
interface ABTestProviderProps {
  ...
  onExposure?: (experimentId: string, variantId: string, metadata?: Record<string, any>) => void;
}
...
export function ABTestProvider({
  ...
  const [assignments, setAssignments] = useState<Map<string, VariantAssignment>>(new Map());
  ...
  const userId = useMemo(() => providedUserId || generateUserId(), [providedUserId]);
  ...
  const contextValue: ABTestContextValue = useMemo(
    ...
    [experiments, assignments, getVariant, trackExposure, isInVariant, refreshExperiments]
  );
