...
const UTM_SECRET = process.env.UTM_SECRET_KEY || 'development-secret-key-replace-in-production'
...
export async function validateUTMToken(utm: string): Promise<ValidationResult> {
  ...
  if (process.env.NODE_ENV === 'development' && process.env.NEXT_PUBLIC_USE_MOCK_PURCHASE === 'true') {
