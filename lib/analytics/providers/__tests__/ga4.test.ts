...
describe('GoogleAnalytics4Provider', () => {
  ...
  describe('initialize', () => {
    it('should load GA4 script and configure', async () => {
      ...
      const ga4Script = Array.from(scripts).find(s => 
        s.src && s.src.includes(`googletagmanager.com/gtag/js?id=${measurementId}`)
      )
    ...
    it('should not initialize without analytics consent', async () => {
      ...
      const scripts = document.querySelectorAll('script')
      const ga4Script = Array.from(scripts).find(s => 
        s.src && s.src.includes('googletagmanager.com')
      )
  ...
  describe('track', () => {
    ...
    it('should not track when not initialized', () => {
      const uninitializedProvider = new GoogleAnalytics4Provider(measurementId, apiSecret)
  ...
  describe('identify', () => {
    ...
    it('should handle identify without traits', () => {
      ...
      expect(window.gtag).not.toHaveBeenCalledWith('set', 'user_properties', expect.any(Object))
    })
  })
