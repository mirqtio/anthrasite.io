...
test.describe('Purchase Page', () => {
  ...
  test('should redirect to homepage without UTM parameter', async ({ page }) => {
  ...
  test('should show purchase page with valid UTM', async ({ page }) => {
    ...
    await expect(page.getByRole('heading', { level: 1 })).toContainText('Your Website Audit Report')
    await expect(page.getByText('$99')).toBeVisible()
    await expect(page.getByRole('button', { name: /Get Your Report Now/i })).toBeVisible()
  })
  ...
  test('should show all included features', async ({ page }) => {
    ...
    await expect(page.getByText(/Complete 50\+ page website audit report/)).toBeVisible()
    await expect(page.getByText(/Technical SEO analysis/)).toBeVisible()
    await expect(page.getByText(/Security vulnerability assessment/)).toBeVisible()
  ...
  test('should handle checkout button click', async ({ page }) => {
    ...
    const checkoutButton = page.getByRole('button', { name: /Get Your Report Now/i })
  ...
  test('should show warning for used UTM tokens', async ({ page }) => {
    ...
    await expect(page.getByText(/This purchase link has already been used/)).toBeVisible()
  })
  ...
  test('should maintain scroll position on navigation', async ({ page }) => {
    ...
    await page.getByText('Get Your Complete Website Audit').scrollIntoViewIfNeeded()
