...
test.describe('Waitlist Signup', () => {
  ...
  async function navigateToWaitlistForm(page: Page) {
    ...
    await page.waitForSelector('input[placeholder="example.com"]', { state: 'visible', timeout: 5000 })
  }
  
  test('should complete waitlist signup with valid domain', async ({ page }) => {
    ...
    await expect(page.getByText(/we'll analyze e2e-test-valid.com/i)).toBeVisible()
  ...
  test('should show suggestion for domain typo', async ({ page }) => {
    ...
    // Input should update
    await expect(page.locator('input[placeholder="example.com"]')).toHaveValue('gmail.com')
  })
  ...
  test('should normalize domain with www prefix', async ({ page }) => {
    ...
    await expect(page.getByText(/we'll analyze e2e-test-www.com/i)).toBeVisible()
  ...
  test('should validate email format', async ({ page }) => {
    ...
    for (const invalidEmail of invalidEmails) {
      ...
      await expect(page.getByRole('button', { name: /join waitlist/i })).toBeDisabled()
    }
  ...
  test('should handle back navigation', async ({ page }) => {
    ...
    await expect(page.locator('input[placeholder="example.com"]')).toHaveValue('back.com')
  })
