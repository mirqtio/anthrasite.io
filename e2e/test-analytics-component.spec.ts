...
test('Analytics Component Rendering Test', async ({ page }) => {
  ...
  const componentCheck = await page.evaluate(() => {
    ...
    const analyticsElements = document.querySelectorAll('[data-analytics], [data-ga], [data-gtag]');
  ...
  const ga4RequestPromise = page.waitForRequest(
    request => request.url().includes('google-analytics.com') || request.url().includes('analytics'),
    { timeout: 5000 }
  ).catch(() => null);
  ...
  console.log('\n=== GA4 Request ===');
  console.log(ga4Request ? `Found: ${ga4Request.url()}` : 'No GA4 requests detected');
});