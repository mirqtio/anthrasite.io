...
test.describe('UTM Parameter Validation', () => {
  ...
  test.describe('Valid UTM flow', () => {
    test('should display purchase page with valid UTM', async ({ page, request }) => {
      // Setup: Create business and generate UTM
      const business = await createTestBusiness()
      const utmUrl = await generateUTMUrl('http://localhost:3000/purchase', business.id)
    ...
    test('should persist purchase mode on homepage with valid UTM', async ({ page }) => {
      const business = await createTestBusiness()
      ...
      try {
        ...
        const siteModeCookieAfter = cookiesAfter.find(c => c.name === 'site_mode')
  ...
  test.describe('Invalid UTM handling', () => {
    test('should redirect to homepage with missing UTM on protected page', async ({ page }) => {
    ...
    test('should redirect for tampered UTM', async ({ page }) => {
      const business = await createTestBusiness()
      const validUrl = generateUTMUrl('http://localhost:3000/purchase', business.id)
  ...
  test.describe('One-time use enforcement', () => {
    test('should prevent reuse of UTM token', async ({ page, request }) => {
      ...
      const utmUrl = generateUTMUrl('http://localhost:3000/purchase', business.id)
