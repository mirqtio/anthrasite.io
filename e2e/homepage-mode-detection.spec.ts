...
test.describe('Homepage Mode Detection', () => {
  ...
  test.describe('Direct visit → Organic mode', () => {
    test('should display organic homepage when visiting without UTM parameters', async ({ page, context }) => {
      ...
      await expect(page.locator('h1')).toContainText('Automated Website Audits That Uncover Untapped Potential')
      await expect(page.locator('text=Join Waitlist')).toBeVisible()
      await expect(page.locator('text=Learn more')).toBeVisible()
      
      // Verify features section is visible
      await expect(page.locator('text=Comprehensive Website Analysis')).toBeVisible()
      ...
      await expect(page.locator('text=Website Audit is Ready')).not.toBeVisible()
      await expect(page.locator('text=Get Your Full Report - $199')).not.toBeVisible()
    ...
    test('should clear purchase mode when visiting without UTM', async ({ page, context }) => {
  ...
  test.describe('Email link → Purchase mode', () => {
    test('should display purchase homepage with valid UTM parameter', async ({ page, context }) => {
      ...
      try {
        ...
        await expect(page.locator('text=Get Your Full Report - $199')).toBeVisible()
        ...
        await expect(page.locator('text=Critical Security Headers Missing')).toBeVisible()
        await expect(page.locator('text=Performance Optimization Needed')).toBeVisible()
    ...
    test('should validate UTM via API when in purchase mode', async ({ page, context, request }) => {
      const business = await createTestBusiness()
      const utmUrl = await generateUTMUrl('http://localhost:3000', business.id)
  ...
  test.describe('Mode persistence across refreshes', () => {
    test('should persist purchase mode across page refreshes', async ({ page, context }) => {
      const business = await createTestBusiness()
      const utmUrl = await generateUTMUrl('http://localhost:3000', business.id)
    ...
    test('should maintain purchase mode when navigating between pages', async ({ page, context }) => {
      const business = await createTestBusiness()
      const utmUrl = await generateUTMUrl('http://localhost:3000', business.id)
    ...
    test('should expire purchase mode after cookie expiration', async ({ page, context }) => {
      const business = await createTestBusiness()
  ...
  test.describe('Invalid UTM parameter handling', () => {
    test('should show error state for malformed UTM', async ({ page }) => {
      ...
      for (const utm of malformedUTMs) {
        ...
        await expect(page.locator('text=This purchase link is not valid')).toBeVisible()
    ...
    test('should show error for tampered UTM signature', async ({ page }) => {
      const business = await createTestBusiness()
      const validUrl = await generateUTMUrl('http://localhost:3000', business.id)
    ...
    test('should gracefully handle missing business data', async ({ page, context }) => {
  ...
  test.describe('Loading states', () => {
    test('should show loading state during mode detection', async ({ page }) => {
  ...
  test.describe('Mobile responsiveness', () => {
    test('should display correctly on mobile devices', async ({ page, context }) => {
      const business = await createTestBusiness()
      const utmUrl = await generateUTMUrl('http://localhost:3000', business.id)
  ...
  test.describe('Edge cases and race conditions', () => {
    test('should handle rapid mode switches gracefully', async ({ page, context }) => {
      const business = await createTestBusiness()
      const utmUrl = await generateUTMUrl('http://localhost:3000', business.id)
    ...
    test('should handle concurrent cookie operations', async ({ browser }) => {
      ...
      try {
        ...
        await expect(page1.locator('h1')).toContainText('Automated Website Audits')
        await expect(page2.locator('h1')).toContainText('Website Audit is Ready')
