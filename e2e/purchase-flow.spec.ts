...
test.describe('Purchase Flow E2E Tests', () => {
  ...
  test.beforeAll(async ({ request }) => {
    ...
    if (ENV !== 'development' || !config.bypassToken) {
      const response = await request.post(`${config.baseUrl}/api/admin/generate-utm`, {
        headers: {
  ...
  test('Purchase page preview mode shows all components', async ({ page }) => {
    ...
    const checkoutButton = page.getByRole('button', { name: /get.*report|checkout/i })
  ...
  test('Purchase flow tracking events fire correctly', async ({ page }) => {
    ...
    const pageViewEvents = analyticsRequests.filter(req => 
      req.postData?.includes('page_view') || 
      req.url.includes('pageview')
    )
  ...
  test('Performance: Purchase page loads within acceptable time', async ({ page }) => {
